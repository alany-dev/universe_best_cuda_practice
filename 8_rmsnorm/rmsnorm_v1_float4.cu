...
__global__ void row_rmsnorm_f32_dim_float4(float *in, float *wei, float *out, int batch, int size, float eps)
{
...
int main()
{
    ...
    for (int i = 0; i < warpup; i++) {
        // Warm-up run
        row_rmsnorm_f32_dim_float4<<<grid, block>>>(d_input, d_weight, d_output, batch, size, eps);
    }
    ...
    // row_rmsnorm_f32_dim_float4<<<grid, block>>>(d_input, d_weight, d_output, batch,
    // size, eps);
    int test_iter = 10;
