...
__global__ void row_rmsnorm_f32_dim_native(float *in, float *wei, float *out, int batch, int size, float eps)
{
...
float compute_max_error(const std::vector<float> &cpu_out, const std::vector<float> &cuda_out, int n)
{
    float max_err = 0.0f;
    for (int i = 0; i < n; ++i) {
        ...
        if (max_err > 1.f) {
            std::cout << "Error at index " << i << ": CPU = " << cpu_out[i] << ", CUDA = " << cuda_out[i]
                      << ", Error = " << err << "\n";
            break;
        }
    }
...
int main()
{
    ...
    for (int i = 0; i < warpup; i++) {
        // Warm-up run
        row_rmsnorm_f32_dim_native<<<grid, block>>>(d_input, d_weight, d_output, batch, size, eps);
    }
    ...
    // row_rmsnorm_f32_dim_native<<<grid, block>>>(d_input, d_weight, d_output, batch,
    // size, eps);
    int test_iter = 10;
