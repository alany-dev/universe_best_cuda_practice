        ...
        瓶颈：
            - 共享内存的 bank conflict：
                ...
                因此该 warp 的 32 个线程同时读取 tile[0][irow], tile[1][irow], …, tile[31][irow]。
                    ...
                    所有线程的地址除以 4 后对 32 取模，(base/4 + i*32 + irow) % 32 = (base/4 + irow) % 32，全部落在同一个 bank。
                    → 32 路 bank conflict，共享内存带宽下降约 32 倍。

            在 Step A (读入) 时： 线程 ID (tx, ty) 负责坐标 (tx, ty)。
            在 Step B (写出) 时： 我们让线程 ID (tx, ty) 去负责坐标 (irow, icol)。
...
void call_transpose(float *d_out, float *d_in, int nx, int ny)
{
    ...
    transpose<32, 32><<<gridSize, blockSize>>>(d_out, d_in, nx, ny);
}
