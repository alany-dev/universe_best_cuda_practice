...
*/
template <unsigned int BLOCK_SIZE> __global__ void reduce(float *g_idata, float *g_odata, int n)
{
...
int main()
{
    ...
    reduce<BLOCK_SIZE><<<num_blocks, BLOCK_SIZE>>>(d_data, d_result, N);
    reduce<BLOCK_SIZE><<<1, num_blocks>>>(d_result, d_final_result, num_blocks);
