...
/*
    最简单的写法：
        每个线程 tx ty 负责每个 Block 内部的 对应坐标数据计算

    A 的 1*k 和 B 的 k*1 计算得到 C 的 1*1

    问题：
        1. 共享内存
        2. 比如说每次求 C 的 某一行的数据，那么此时对应的 A 的 1*k 同一行需要反复读取 N 遍。
*/

__global__ void sgemm(float *a_ptr, float *b_ptr, float *c_ptr, const int M, const int N, const int K)
{
    int x = blockIdx.x * blockDim.x + threadIdx.x;  // 横坐标
    int y = blockIdx.y * blockDim.y + threadIdx.y;  // 纵坐标
...
int main()
{
    ...
    sgemm<<<Grid, Block>>>(matrix_a_device, matrix_b_device, matrix_c_device, m, n, k);
