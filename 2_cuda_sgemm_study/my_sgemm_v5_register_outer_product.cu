...
__global__ void sgemm(float *a_ptr, float *b_ptr, float *c_ptr, const int M, const int N, const int K)
{
    ...
    int bx = blockIdx.x;
    int by = blockIdx.y;
    // 重排，每个 Block 线程块的线程数 还是 8*32。
    // 我需要每个线程在 实际 外积的时候，每个线程负责 相邻的 2*2 的数据。
    int tid = ty * blockDim.x + tx;
    // 每个线程处理 2*2，那么就会有 0-15 x 0-15 个线程
    int cty = tid / 16; // 用于定位2x2数据的行基址
    int ctx = tid % 16; // 用于定位2x2数据的列基址
    a_ptr   = &a_ptr[by * BM * K];
    b_ptr   = &b_ptr[bx * BN];
    ...
    float temp[RM][RN] = {0.f}; // 每个线程处理连续的四个数的计算，所以temp是一维的

    for (int k = 0; k < K; k += BK) {
        // global ---> shared  float4 读取优化
        FETCH_FLOAT4(shared_a_ptr[ty][tx * 4]) = FETCH_FLOAT4(a_ptr[ty * K + tx * 4]);
        FETCH_FLOAT4(shared_b_ptr[ty][tx * 4]) = FETCH_FLOAT4(b_ptr[ty * N + tx * 4]);
...
#pragma unroll
            for (int i = 0; i < RM; i++) {
                a_reg[i] = shared_a_ptr[i + cty * RM][t];
            }
#pragma unroll
            for (int j = 0; j < RN; j++) {
                b_reg[j] = shared_b_ptr[t][ctx * RN + j];
            }
    ...
    for (int i = 0; i < RM; i++) {
        for (int j = 0; j < RN; j++) {
            c_ptr[N * (i + cty * RM) + ctx * RN + j] = temp[i][j];
        }
    }
}
