...
__global__ void mysgemm(int M, int N, int K, float alpha, float *A, float *B, float beta, float *C)
{
    ...
    const unsigned int BM = BLOCK_SIZE;
    const unsigned int BN = BLOCK_SIZE;
    const unsigned int BK = BLOCK_SIZE;
...
int main()
{
    ...
    for (int N : sizes) {
        ...
        try {
            ...
            for (int i = 0; i < warpup_time; ++i) {
                mysgemm<32><<<gridDim, blockDim>>>(N, N, N, alpha, d_A, d_B, beta, d_C_v1);
            }
            cudaDeviceSynchronize();
